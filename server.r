...
function(input, output, session) {
  ...
  ## calcul des cinétiques
    kinetics<-function(densren,eatAT,N,K,tpieg){
    if(!is.null(tpieg)){
    ...
    for(i in 2:durerep) {
      ...
      if(i%in%tpieg[,1]) {
        Ntr1p[i]<-Ntr1p[i]*(1-tpieg[which(tpieg[,1]==i),2]) #  femelles piégées
  ...
  dens2<-reactive(kinetics(input$densren,input$eatAT,input$N,input$K,{tabpieg()})) # calcule les cinétiques
  ...
  output$plot1 <- renderPlot({ # envoie le graphe
    ...
    mtext("Attention: le modèle ne prend pas en compte l'effet possible du traitement sur les prédateurs",side=1,line=5,adj=0,cex=0.8)
